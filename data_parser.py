...
if __name__ == '__main__':
    ...
    conversation_dict = {}
    #Create a dictionary with all utterances
    with codecs.open('data/tokenized_words.txt', "w", encoding='utf-8', errors='ignore') as f:
        for line in raw_movie_lines:
            line = line.split(' +++$+++ ')
            line_ID = line[0]
            utterance = line[-1]
            conversation_dict[line_ID] = utterance
            utterance = " ".join([refine(w) for w in utterance.lower().split()])
            f.write(utterance + '\n')
    pickle.dump(conversation_dict, open('data/conversation_dict', 'wb'), True)