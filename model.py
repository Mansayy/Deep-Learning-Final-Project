...
class Seq2Seq_chatbot():
    ...
    def build_model(self):
        ...
        for i in range(0, self.n_encode_lstm_step):
            ...
            with tf.variable_scope("LSTM1"):
                output1, state1 = self.lstm1(wordvec_emb[:, i, :], state1)                  #Output from 1st LSTM layer

            with tf.variable_scope("LSTM2"):
                output2, state2 = self.lstm2(tf.concat([padding, output1], 1), state2)      #Output from 2nd LSTM layer

        #Decoding Stage 
        for i in range(0, self.n_decode_lstm_step):
            with tf.device("/cpu:0"):
                current_embed = tf.nn.embedding_lookup(self.Wemb, caption[:, i])          #Getting the current embedding

            tf.get_variable_scope().reuse_variables()

            with tf.variable_scope("LSTM1"):
                output1, state1 = self.lstm1(padding, state1)           #Output from 1st LSTM layer

            with tf.variable_scope("LSTM2"):
                output2, state2 = self.lstm2(tf.concat([current_embed, output1], 1), state2)           #Output from 2nd LSTM layer
            ...
            cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=logit_words, labels=onehot_labels)       #Calculating the cross entropy loss
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
              train_op = tf.train.AdamOptimizer(self.lr).minimize(loss)     #Optimizer
    ...
    def build_generator(self):
        ...
        for i in range(0, self.n_decode_lstm_step):
            ...
            with tf.variable_scope("LSTM1"):
                output1, state1 = self.lstm1(padding, state1)           ##Output of 1st LSTM layer in encoding

            with tf.variable_scope("LSTM2"):
                output2, state2 = self.lstm2(tf.concat([current_embed, output1], 1), state2)           #Output of 2nd LSTM layer in encoding
