...
def train():
    wordtoix, ixtoword, bias_init_vector = data_parser.preProBuildWordVocab(word_count_threshold=word_count_threshold)          #Data cleaning
    word_vector = KeyedVectors.load_word2vec_format('model/word_vector.bin', binary=True)
    #Defing the Seq2Seq model
    ...
    train_op, tf_loss, word_vectors, tf_caption, tf_caption_mask, inter_value = model.build_model()         #Building the model
    ...
    for epoch in range(start_epoch, epochs):
        n_batch = dr.get_batch_num(batch_size)      #Getting number of batches
        for batch in range(n_batch):
            start_time = time.time()

            batch_X, batch_Y = dr.generate_training_batch(batch_size)           #Creating Batch X and Batch Y
